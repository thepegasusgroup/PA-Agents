...
const SCOPES = [
  ...
  'https://www.googleapis.com/auth/userinfo.profile',  // name + picture for Contacts header
...
function init(mainWindow) {
  ...
  // Load credentials (lives at the repo root, one level above electron/)
...
async function startOAuthFlow() {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Start local HTTP server to catch the redirect from the user's browser
      ...
      if (url.pathname === '/oauth/callback') {
        ...
        if (code) {
          try {
            ...
            res.writeHead(200, { 'Content-Type': 'text/html' });
            res.end('<html><body><h2>Authentication successful!</h2><p>You can close this tab and return to Pegasus Agents.</p></body></html>');
            if (timeoutHandle) clearTimeout(timeoutHandle);
            try { server.close(); } catch (e) {}
            resolve();
          } catch (e) {
    ...
    server.listen(REDIRECT_PORT, () => {
      // Open the user's default browser — lets them use existing Google sessions
      ...
      // Safety net — if the user never completes the flow, close the server
      // after 5 minutes so the port doesn't stay bound forever.
...
// User identity from the OAuth2 userinfo endpoint — name + picture for the
// Contacts header. Requires userinfo.profile scope; users authed before that
// scope was added will need to re-sign-in. Falls back gracefully via the IPC
// wrapper if the call fails.
async function getUserInfo() {
  await ensureValidToken();
  const token = store.get('gmail.access_token');
  const data = await apiGetHost('www.googleapis.com', '/oauth2/v2/userinfo', token);
  if (data.error) {
    throw new Error(data.error.message || data.error_description || JSON.stringify(data.error));
  }
  return {
    email: data.email || '',
    name: data.name || '',
...
function apiGet(path, token) {
  return apiGetHost('gmail.googleapis.com', path, token);
}

// Host-aware GET — used by getUserInfo which targets www.googleapis.com.
function apiGetHost(hostname, path, token) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname,
      path,
...
module.exports = { init, isAuthenticated, startOAuthFlow, logout, listMessages, getMessage, getProfile, getUserInfo };
